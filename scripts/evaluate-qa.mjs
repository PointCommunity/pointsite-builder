...
try {
  ...
  if (!profilePath || !evidencePath || extra.length)
    throw new Error('Usage: node scripts/evaluate-qa.mjs <profile.json> <evidence.json>');
  ...
  const result = evaluateQA(profile.qa, evidence.candidate, evidence.records, {
    ...
    cleanupResolutions: evidence.cleanupResolutions,
  });
