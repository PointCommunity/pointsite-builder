...
const CONNECTIONS = {
  ...
  fields:
    '... on ProjectV2FieldCommon { id name } ... on ProjectV2SingleSelectField { options { name } }',
...
export async function collectConnection(fetchPage) {
  ...
  do {
    ...
    if (
      ...
      throw new Error('incomplete Project connection');
    expected ??= connection.totalCount;
    if (expected !== connection.totalCount)
      throw new Error('Project connection changed during pagination; retry audit');
    for (const node of connection.nodes) {
      if (!node || typeof node.id !== 'string' || !node.id)
        throw new Error('unreadable Project node');
    ...
    if (typeof cursor !== 'string' || !cursor || cursors.has(cursor) || !connection.nodes.length)
...
export async function fetchProjectSnapshot(profile, ghJson) {
  ...
  if (
    !project?.id ||
    project.number !== profile.project.number ||
    project.owner?.login?.toLowerCase() !== profile.project.owner.toLowerCase() ||
    !['User', 'Organization'].includes(project.owner.type) ||
    typeof project.public !== 'boolean' ||
    typeof project.title !== 'string'
  )
    throw new Error('Project identity unavailable or mismatched');
  const collections = {};
  for (const [name, selection] of Object.entries(CONNECTIONS)) {
    collections[name] = await collectConnection(async (cursor) => {
      ...
      if (result.errors?.length)
        throw new Error('GitHub returned partial Project data; audit cannot pass');
      return result.data?.node?.[name];
    });
  }
  const readback = await ghJson(args);
  if (
    ['id', 'number', 'title', 'public'].some((key) => readback?.[key] !== project[key]) ||
    readback?.owner?.login !== project.owner.login
  )
    throw new Error('Project identity changed during audit; retry');
  return {
    project: project.url,
    snapshot: {
      title: project.title,
      ...
      fields: collections.fields.map((field) => ({
        name: field.name,
