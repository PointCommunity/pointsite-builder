...
export function validateAdoptionContract({ agents = '', adoptionSkill = '' } = {}) {
  ...
  if (!/ask one concise question[\s\S]*wait before target or Project mutation/i.test(agents)) {
    errors.push(
      'AGENTS.md must require the agent to ask once and wait before target or Project mutation',
    );
  }
...
export function validateProjectBlueprint(blueprint) {
  ...
  const workflows = new Map(
    (blueprint.workflows ?? []).map((workflow) => [workflow.name, workflow]),
  );
...
export function compareProjectSnapshot(blueprint, snapshot, identity = blueprint.workingExample) {
  ...
  for (const expected of blueprint.views) {
    ...
    else if (actual.layout !== expected.layout)
      errors.push(`view layout mismatch for ${expected.name}`);
  }
...
export async function validateRepository(rootPath, { requireConfig = true } = {}) {
  ...
  for (const examplePath of await walkFiles(path.join(root, 'blueprints', 'qa'))) {
    if (!examplePath.endsWith('.json')) continue;
    try {
      validateQA(JSON.parse(await readFile(examplePath, 'utf8')));
    } catch (error) {
      errors.push(`invalid ${path.relative(root, examplePath)}: ${error.message}`);
    }
  }
  ...
  if (!sameValues(adapterNames, skillNames))
    errors.push('Claude adapter registry must match canonical skills');
  ...
  for (const name of skillNames) {
    ...
    if (!frontmatter) errors.push(`${skillRelative} has invalid frontmatter`);
      ...
      if (frontmatter.name !== name)
        errors.push(`${skillRelative} frontmatter name must equal ${name}`);
      if (!frontmatter.description)
        errors.push(`${skillRelative} frontmatter description is required`);
    }
    ...
    try {
      if ((await lstat(adapterPath)).isSymbolicLink())
        errors.push(`${adapterRelative} must be a regular file`);
    } catch {
    ...
    if (!adapter.includes(canonicalTarget))
      errors.push(`${adapterRelative} must link to the canonical skill`);
  }
