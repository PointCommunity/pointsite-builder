...
async function assertSafeDestination(root, destination) {
  ...
  if (!relative || relative.startsWith('..') || path.isAbsolute(relative))
    throw new Error('unsafe adoption destination');
  let current = root;
  for (const part of relative.split(path.sep)) {
    current = path.join(current, part);
    try {
      if ((await lstat(current)).isSymbolicLink())
        throw new Error(`refusing destination symlink: ${current}`);
    } catch (error) {
...
export async function planAdoption({ sourceRoot, targetRoot, profile, reconciliation }) {
  ...
  if (reconciliation && (reconciliation.version !== 1 || !reconciliation.files))
  ...
  for (const candidate of candidates) {
    ...
    if (current.equals(candidate.content)) plan.identical.push(candidate);
      ...
      if (
        review?.sourceSha256 === digest(candidate.content) &&
        review.targetSha256 === digest(current) &&
...
export async function applyAdoptionPlan(plan) {
  ...
  let created = 0;
  if ((await realpath(plan.targetRoot)) !== plan.targetRoot)
    throw new Error('adoption root changed after planning');
  for (const item of [...plan.create, ...plan.identical, ...(plan.reconciled ?? [])])
    await assertSafeDestination(plan.targetRoot, item.targetPath);
  for (const item of [...plan.identical, ...(plan.reconciled ?? [])]) {
    if (!(await readFile(item.targetPath)).equals(item.content))
      throw new Error(`file changed after planning: ${item.relativePath}`);
  }
