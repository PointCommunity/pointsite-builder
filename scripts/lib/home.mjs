...
export async function resolveHome(anchor) {
  ...
  const root = await realpath(
    (await run('git', ['-C', location, 'rev-parse', '--show-toplevel'])).stdout.trim(),
  );
  ...
  if (location !== root && !location.startsWith(root + path.sep))
    throw new Error('home anchor outside Git root');
  ...
  return {
    ...
    profilePath: path.join(root, 'pipeliner.config.json'),
  };
}
export async function verifyRepository(target, profile) {
  const root = await realpath(target);
  const git = async (...args) =>
    (await run('git', ['-C', root, ...args], { maxBuffer: 1024 * 1024 })).stdout.trim();
  if ((await realpath(await git('rev-parse', '--show-toplevel'))) !== root)
    throw new Error('adoption target must be its own Git root');
  const { owner, name } = profile.repository ?? {};
  if (![owner, name].every((value) => typeof value === 'string' && /^[\w.-]+$/.test(value)))
    throw new Error('home repository identity missing');
  ...
  for (const args of [
    ...
    if (
      !remotes.length ||
      remotes.some((remote) => repositoryOf(remote) !== expected.toLowerCase())
    )
      throw new Error('home remote identity mismatch or unsupported remote');
  }
