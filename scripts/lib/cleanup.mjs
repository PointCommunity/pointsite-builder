...
export async function removeTestWorkspace(workspace) {
  ...
  if (!path.basename(directory).startsWith('pipeliner-run-'))
    throw new Error('invalid workspace ownership boundary');
  if ((await lstat(directory)).isSymbolicLink() || (await realpath(directory)) !== directory)
    throw new Error('refusing symlink workspace or ancestor');
  if (
    typeof workspace.token !== 'string' ||
    (await readFile(path.join(directory, marker), 'utf8')) !== workspace.token
  )
...
export function cleanupComplete(cleanup) {
  return (
    ...
    typeof cleanup.evidence === 'string' &&
    cleanup.evidence.trim() !== '' &&
    Array.isArray(cleanup.resources) &&
    cleanup.resources.every(
      (resource) =>
        ['process', 'container', 'image', 'volume', 'workspace', 'artifact'].includes(
          resource?.kind,
        ) &&
        ['id', 'run', 'owner', 'evidence'].every(
          (key) => typeof resource[key] === 'string' && resource[key].trim(),
        ) &&
