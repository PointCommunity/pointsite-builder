...
function ghJson(args) {
  ...
  if (result.error || result.status !== 0)
    throw new Error(
      `GitHub Project read failed: ${result.error?.message ?? (result.stderr || result.stdout).trim()}`,
    );
...
async function main() {
  ...
  const blueprint = JSON.parse(
    await readFile(new URL('../blueprints/github-project.json', import.meta.url), 'utf8'),
  );
  ...
  console.log(
    JSON.stringify(
      { ...result, visibilityPolicy: profile.project.visibility ?? 'observed; unchanged', errors },
      null,
      2,
    ),
  );
