...
- The Builder product can publish public-site candidates to `PointCommunity/pointsite-staging`; that content workflow is separate from deployment of the Builder application. Canary may explicitly publish Staging candidates, but public Production publication and rollback are disabled there. Canary drafts, roles, jobs and approvals are never copied into Production during application promotion.
...
- Every Canary handoff must link to `https://builder-canary.eaglepass.io`, identify source/tree, image digest and GitOps configuration, and provide change-specific PM testing before requesting Production approval. Every production Showcase handoff must link directly to `https://builder.eaglepass.io`, identify the exact released candidate, provide prerequisites, numbered actions, expected results, and focused regression checks, and end with `Approved to complete Issue #<number>` in its own fenced code block for easy copying. Agent QA and live verification never substitute for PM review and testing.
