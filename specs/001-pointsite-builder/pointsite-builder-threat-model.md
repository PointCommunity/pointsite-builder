...
1. **Browser to Cloudflare Access**: HTTPS | Auth: GitHub identity | Validation: Access policy | Rate: Cloudflare edge plus app controls.
2. **Access to Builder Worker**: HTTPS/internal edge | Auth: signed Access assertion | Validation: issuer, audience, expiry, signature.
3. **Worker to D1/R2**: Cloudflare bindings | Auth: platform binding | Validation: typed repositories, parameterized SQL, object policy.
...
5. **Candidate to Staging Worker**: Git commit/build/static assets | Auth: Cloudflare Access for requests | Validation: schema, renderer, checks, exact commit.
...
2. Access policy, team domain, and application audience (high, configuration).
...
7. Worker, D1, R2, and GitHub quotas (medium, availability).
...
| ----- | --------------------- | --------------------- | ---------------------- | ----------------------------------- | -------- | ------------------------------------------------------------------------------------ |
| TM-01 | Access to Worker      | Roles, drafts         | Spoofing/Elevation     | Bypass identity or role             | Critical | Forged or unvalidated identity data could grant administration.                      |
...
| TM-04 | Media upload          | R2, browser           | Tampering/DoS          | Store active or oversized content   | High     | Deceptive MIME, image bombs, and repeated uploads consume resources.                 |
...
- Validate Access JWT signature, issuer, audience, expiry, and email; perform D1 role lookup on every request.
...
- Enforce image magic bytes, MIME, extension, byte and dimension bounds; random R2 keys, private reads, per-user quotas.
...
- [Confirmed by prior user decisions] Builder access is limited to approved administrators behind Cloudflare Access.
...
- [Unconfirmed external state] Exact Cloudflare account, Zero Trust team domain, approved identities, and zone permissions remain to be discovered during authenticated setup.
