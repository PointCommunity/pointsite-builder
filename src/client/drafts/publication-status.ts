...
export function publicationExplanation(status: DraftPublicationStatus | undefined) {
  const baseline =
    status?.state === 'published'
      ? 'This draft has the current public site baseline.'
      : status?.state === 'behind'
        ? 'The public site has replaced this draft’s known baseline. Reuse does not require a merge.'
        : 'This draft’s public site baseline cannot be verified. Do not assume it is live.';
  const source =
    status?.sourceTarget === 'staging'
      ? ' This draft was copied from Staging. Staging-only content is not necessarily live on the public site.'
      : status?.sourceTarget === 'production'
        ? ' This draft was copied from the public site.'
