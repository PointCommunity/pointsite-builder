...
- Stop for an unanswered material question, pending required PM Testing/approval, missing access/compatible host or actual repository control. After asking a question, stop task work until the Human replies, without polling or timers. Do not start another Issue after completion unless requested.

## Questions and approval

Follow [Builder questions](../../pipeliner-maintain/references/questions.md) and the root structured clarification rule. Preserve answers and stop task work until the Human replies. The generic `clarificationDecision` helper returns `endTurn: true` for an unanswered question, without a deadline or auto-answer; its message channel is a reference default subordinate to Builder controls. Tool timeouts never supply a PM response.
