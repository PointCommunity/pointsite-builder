...
- Keep the nine `pointsite-builder-*` workflow packages separate from the five approved shared design packages. Shared packages may guide implementation or review but must not weaken, replace, or duplicate pipeline authority. The recovery skill owns stuck publication incidents, the review skill owns homelab Canary, and the release skill owns approved exact-source Linode Production deployment.
...
- Keep shared invariants in `.agents/pointsite-builder-pipeline-policy.html`, with Dark Mode styling. Preserve the Project schema, agent-owned movement, Canary-first exact-candidate approval, Linode Production source/tree identity, independent storage, backup and rollback retention, homelab Canary routing, and public PointSite separation.
- Preserve lean closeout: routine Issue completion uses concise Issue or PR evidence plus a linked production Showcase in chat. It must not create or open a duplicate acceptance report unless the PM explicitly requests that artifact.
- Preserve the sole product-work PM gate: agent QA and Canary deployment precede PM testing and exact approval of the unchanged Canary candidate. After that approval, merge and promote the same digest; verified Production and full Issue acceptance permit Done and closure without another PM approval.
...
- Test realistic positive and negative routing prompts when trigger descriptions change. Never weaken Issue-creation authorization, the sole post-Canary exact-candidate approval, the active-Issue invariant, full local QA, exact-main deployment, rollback, or Production Showcase to make a test pass.
