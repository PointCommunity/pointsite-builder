...
1. Read upstream and target instructions completely. Preserve dirty work and stronger constraints. Manually merge only adoption-authorized changes; never overwrite unknown changes.
   Review root instructions, provider imports/adapters and prompts for question conflicts. The dry-run's `questionConflicts` blocks writes even with matching review hashes. Preserve the PM-approved exact Builder structured-question clause exception; reconcile other conflicting text to stop task work after asking. Detection is bounded; manually inspect host instructions and disclose conflicts without changing global settings.
