...
Read `AGENTS.md`, then inspect the live `PointCommunity/pointsite-builder` repository, its open and closed Issues, open and merged pull requests, available labels, milestones, and any connected GitHub Projects before drafting. Treat all GitHub metadata as time-sensitive.
...
5. Propose metadata using only live repository options. Prefer one primary classification label such as `bug`, `enhancement`, `documentation`, or `question`, plus directly applicable supplemental labels such as `accessibility`. Do not invent labels, milestones, Project fields, priorities, or statuses. Leave assignee, milestone, and Project placement unset unless the PM approves a currently available value.
...
8. Read the created Issue and any Project item back from GitHub. Verify the URL, open state, title, byte-for-byte body, labels, assignee, milestone, and Project fields against the approved proposal, then run a final duplicate check.
