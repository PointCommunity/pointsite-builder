...
6. Add concise release evidence to the PR or Issue: source commit/tree, GitHub Quality run, prior and new Cloudflare versions, migrations, deploy command result, live verifier output, and browser evidence or its supported-access limitation. Keep the Issue open, assigned, and In Review; production health does not replace PM testing. Do not duplicate this evidence into a standalone report or open a local summary artifact unless the PM explicitly requested that artifact.
